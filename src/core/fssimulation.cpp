    ...
    int &nEnemyMax,double &gLimit,const double &timeRemain,YSBOOL allowAAM,YSBOOL jet,YSBOOL ww2)
{
	...
	while((airSeek=FindNextAirplane(airSeek))!=NULL)
	{
		if(airSeek->Prop().IsActive()==YSTRUE && airSeek->iff!=FS_IFF0 && airSeek->iff!=FS_IFF1)
		{
...
void FsSimulation::GenerateEnemyAirplane(
		...
		printf("Generating enemy airplane\n");
		printf("Target IFF: %d\n",targetIFF);
		printf("Attacker IFF: %d\n",attackerIFF);
		printf("Interceptor: %s\n",interceptorAircraft);
		...
		while((airSeek=FindNextAirplane(airSeek))!=NULL)
		{
			if(airSeek->Prop().IsActive()==YSTRUE && airSeek->iff==targetIFF && airSeek->iff!=FS_IFF0)
			{
				refPos=airSeek->GetPosition();
				airExist=YSTRUE;
		...
		if(airExist==YSTRUE){
			...
			//There is a valid target, how many people on the interceptor IFF are there (we don't need as many
			//interceptors if there are already a lot of them, or players on that IFF playing the interceptor role)
			nCurrentEnemy=0;
			airSeek=NULL;
			while((airSeek=FindNextAirplane(airSeek))!=NULL)
			{
				if(airSeek->Prop().IsActive()==YSTRUE && airSeek->iff==attackerIFF)
				{
					nCurrentEnemy++;
				}
			}
		...
		//If there are less than 'nEnemy' interceptors, generate 1.

		if(nCurrentEnemy<nEnemy){
			double radial;
			radial=double(rand()%360)*YsPi/180.0;
			int i;
			YsVec3 diff,neoPos;
			for (i=nCurrentEnemy; i<nEnemy; i++){
				//Generate AI
				diff.Set(sin(radial),0.0,cos(radial));
				diff=diff*8000.0;
				neoPos=pos+diff;
				...
				FsAirplane *neo;
				neo = world->AddAirplane(interceptorAircraft, YSFALSE);
				if(neo!=NULL)
					{
						printf("Adding interceptor\n");
						neo->SendCommand("UNLOADWP");
						if(2<=neo->Prop().GetMaxNumWeapon(FSWEAPON_AIM9X))
						{
							neo->SendCommand("LOADWEPN AIM9X 2");
							neo->SendCommand("LOADWEPN AIM9 2");
						}
						else
						{
							neo->SendCommand("LOADWEPN AIM9 4");
						}
						...
						{
							netServer->BroadcastAddAirplane(neo,FSNET_REMOTE);
							printf("Broadcasting add airplane\n");
						}
				}
				radial+=YsDegToRad(40.0);
			}
